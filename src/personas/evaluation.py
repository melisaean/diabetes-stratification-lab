...
def reconstruction_metrics(
    ...
    """Compute MSE reconstruction for the AE and a PCA baseline."""
    import torch
    model.eval()
    ...
    return {
        "ae_test_mse": ae_mse,
        "pca_test_mse": pca_mse,
        "improvement_pct": (pca_mse - ae_mse) / pca_mse * 100,
    }


def cluster_sweep(latent: np.ndarray, k_range: range = range(4, 13)) -> pd.DataFrame:
    """Run KMeans for each k and compute quality metrics."""
...
def bootstrap_stability(latent: np.ndarray, labels: np.ndarray, n_boot: int = 50, seed: int = 42) -> float:
    """Bootstrap stability: mean ARI across resamples."""
...
def downstream_scores(
    ...
    """Benchmark readmission prediction across feature sets."""
    from sklearn.model_selection import StratifiedKFold

    datasets = {
        "scaled_features": X_scaled,
        "ae_fingerprints": fingerprints,
...
def persona_profiles(
    ...
    """Compute per-persona effect sizes vs cohort mean."""
