...
from personas.artifacts import ModelArtifacts, save_artifacts, _hash_settings
from datetime import datetime, timezone


def run_pipeline(settings: Settings, device: str | None = None) -> Dict[str, Path]:
    """Execute the full pipeline and return paths to produced artefacts.

    Steps:
      1. Load raw data → drop high-null columns → save bronze base CSV
      2. Feature engineer (fit on train split, transform all)
      3. Train autoencoder with early stopping
      4. Extract fingerprints on full matrix
      5. K-Means clustering → persona_id
      6. UMAP projection → map_coords.csv
      7. Save all artifacts (model, engineer, manifest)
    """
    if device is None:
    ...
    engineer.fit(df_raw)  # fit on raw (before gender filter in transform)
    ...
    # Align base with engineered rows (gender filter may remove some)
    ...
    X_train, X_val = train_test_split(
        X_np, test_size=settings.val_split, random_state=settings.seed
    )
    ...
    # ── Step 7: Save final base + artefacts ───────────────────────────────
    ...
    # Manifest already saved; add git sha if available
    ...
    return {
        "base": proc_dir / "diabetic_cleaned_base.csv",
        "fingerprints": proc_dir / "patient_fingerprints.csv",
        "map_coords": proc_dir / "map_coords.csv",
        "features": proc_dir / "diabetic_features_scaled.csv",
        "model": models_dir / "autoencoder.pt",
        "engineer": models_dir / "engineer.joblib",
